...
public final class LinkLineRenderer {
    ...
    private static void render(WorldRenderContext context) {
        ...
        Set<Link> links = cachedLinks(world, player.getChunkPos());
        if (links.isEmpty()) {
            return;
        }
    ...
    /** How long a gathered set of links is reused before rescanning. */
    private static final long RESCAN_INTERVAL_MS = 250;

    private static Set<Link> cachedLinks = Set.of();
    private static long nextScan;
    private static ClientWorld scannedWorld;
    private static ChunkPos scannedCenter;

    /**
     * The link set, rescanned a few times a second instead of every frame.
     * The scan walks every block entity in a 13x13 chunk area, which at frame
     * rate was by far the most expensive thing the Speaker Link did; links
     * only change when the player edits them, so a quarter second of lag on
     * a freshly drawn beam is imperceptible.
     */
    private static Set<Link> cachedLinks(ClientWorld world, ChunkPos center) {
        long now = System.currentTimeMillis();
        if (now >= nextScan || world != scannedWorld || !center.equals(scannedCenter)) {
            nextScan = now + RESCAN_INTERVAL_MS;
            scannedWorld = world;
            scannedCenter = center;
            cachedLinks = collectLinks(world, center);
        }
        return cachedLinks;
    }

    /** Drops the cached scan (leaving a world, or when links were just edited). */
